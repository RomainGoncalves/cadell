function IGallery(){
    ...
    var thumbW = parseFloat(globalJS.thumbW);
    var thumbH = parseFloat(globalJS.thumbH);
    var backButton;
    var loadingUI;
    
    this.init=function(){        
        backButton = $('#iGalBackButton a');
    ...
    function addGalleries(){
        ...
        for(var i=0;i<galleries.length;i++){
            for(var j=0;j<galleries[i].length;j++){
                ...
                item.css('opacity', 0);
                if(i==galleries.length-1 && j==galleries[i].length-1){
                    showOffiGallery();
                }
            }
        }        
    ...
    function displayGalleries(){
        ...
        var itemsPossibleNo = Math.floor(containerWidth/thumbW);
        var totalRemainingSpace = (containerWidth-(thumbW*itemsPossibleNo));
                
        if(containerWidth>thumbW){
            if(totalRemainingSpace/(itemsPossibleNo+1)>spacingX){
                //it fits                
                spacingX = totalRemainingSpace/(itemsPossibleNo+1);            
            }else{
                //doesent fit                 
                 totalRemainingSpace = (containerWidth-(thumbW*(itemsPossibleNo-1)));
                 spacingX = totalRemainingSpace/((itemsPossibleNo-1+1)); 
            }                    
        }else{
            spacingX = (containerWidth-thumbW)/2;
        }
        ...
        for(var i=0;i<galleries.length;i++){
            ...
            if(i<galleries.length-1){
                $('.iGalleryContainer').css('height', py+thumbH+'px');
            }
        ...
        for(var i=0;i<galleries.length;i++){
            ...
            if(galleryIndex==i){
                ...
                $('html, body').stop().animate({
                    scrollTop: $('.iGalleryContainer').offset().top-35
                 }, 400);
